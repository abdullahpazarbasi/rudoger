...
public sealed class StockItemAggregateTests
{
    ...
    {
        StockItemAggregate stock = Open(10);
        stock.Receive(5, "receipt", "correlation", Guid.CreateVersion7());
        stock.Adjust(-2, "adjust", "correlation", Guid.CreateVersion7());
        stock.Deduct(3, "deduct", "correlation", Guid.CreateVersion7());
    ...
    public void AdjustmentAndDeductionCannotViolateAvailableStock()
    {
        ...
        Assert.Throws<DomainException>(() => stock.Adjust(-3, "adjust", "correlation", Guid.CreateVersion7()));
        Assert.Throws<ConflictException>(() => stock.Deduct(3, "deduct", "correlation", Guid.CreateVersion7()));
    }
    ...
    [Fact]
    public void ReservationSourceEventIsIdempotentWhileActive()
    {
        StockItemAggregate stock = Open(10);
        Guid orderId = Guid.CreateVersion7();
        Guid sourceEventId = Guid.CreateVersion7();
        stock.Reserve(4, orderId, "correlation", sourceEventId);
    ...
    {
        StockItemAggregate stock = Open(10);
        Assert.Throws<DomainException>(() => stock.Receive(0, "key", "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => stock.Adjust(0, "key", "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => stock.Reserve(1, Guid.Empty, "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => stock.Receive(1, string.Empty, "correlation", Guid.CreateVersion7()));
    }
    ...
    public void OpenValidatesIdentityQuantityAndIdempotencyKey()
    {
        Assert.Throws<DomainException>(() => StockItemAggregate.Open(
            Guid.Empty, Guid.CreateVersion7(), "EA", 0, "key", "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => StockItemAggregate.Open(
            Guid.CreateVersion7(), Guid.Empty, "EA", 0, "key", "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => StockItemAggregate.Open(
            Guid.CreateVersion7(), Guid.CreateVersion7(), "EA", -1, "key", "correlation", Guid.CreateVersion7()));
        Assert.Throws<DomainException>(() => StockItemAggregate.Open(
            Guid.CreateVersion7(), Guid.CreateVersion7(), "EA", 0, string.Empty, "correlation", Guid.CreateVersion7()));
    }

    [Fact]
    public void ManualMovementsAreIdempotentBySourceEvent()
    {
        StockItemAggregate stock = Open(10);
        Guid receipt = Guid.CreateVersion7();
        Guid adjustment = Guid.CreateVersion7();
        Guid deduction = Guid.CreateVersion7();
        stock.Receive(1, "receipt", "correlation", receipt);
        stock.Receive(1, "receipt", "correlation", receipt);
        stock.Adjust(1, "adjust", "correlation", adjustment);
        stock.Adjust(1, "adjust", "correlation", adjustment);
        stock.Deduct(1, "deduct", "correlation", deduction);
        int version = stock.Version;
        stock.Deduct(1, "deduct", "correlation", deduction);
    ...
    {
        StockItemAggregate stock = Open(10);
        Assert.Throws<DomainException>(() => stock.Receive(1, "key", "correlation", Guid.Empty));
        Assert.Throws<DomainException>(() => stock.Receive(1, "key", string.Empty, Guid.CreateVersion7()));
        ...
        Guid sourceEventId = Guid.CreateVersion7();
        var data = new StockMovementData(movementId, 1, 2, "ORDER", referenceId, "key", "correlation", sourceEventId);
        ...
        Assert.Equal(sourceEventId, data.SourceEventId);
    }
